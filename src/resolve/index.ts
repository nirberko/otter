...
export async function resolve(raw: string): Promise<Resolved> {
  ...
  if (kind === 'github') {
    ...
    if (refspec) {
      await execa('git', ['fetch', '--depth', '1', 'origin', refspec], { cwd: dir })
  ...
  if (kind === 'npm') {
    ...
    const { stdout } = await execa('npm', ['pack', ref, '--json'], { cwd: dir })
    const packed = JSON.parse(stdout) as { filename: string; version: string }[]
