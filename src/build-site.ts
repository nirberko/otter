...
<footer>Generated by <a href="https://github.com/nirberko/otter">otter</a> · scores reflect automated static + metadata analysis, not a guarantee.</footer>
</div></body></html>`;
}
...
function indexPage(rows: CatalogRow[]): string {
	...
	const banner = stats.outdated
		? `<div class="banner">⚠ ${stats.outdated} of ${scored} scored results predate the current detection (otter v${SCANNER_VERSION}) and are queued for re-scan.</div>`
		: "";
...
function embedHtml(row: CatalogRow): string {
	...
	const md = `[![otter](${svg})](${page})`;
	return `<h2>Embed this badge</h2>
<p class="sub">Show your otter score in your README:</p>
<p><img src="../badge/${esc(row.slug)}.svg" alt="otter score"></p>
...
function detailPage(row: CatalogRow, report: ScanReport): string {
	...
	const callout = isOutdated(row, CURRENT_CHECKS)
		? `<div class="callout">⚠ Scored with otter v${esc(row.scannerVersion)}; detection has changed since (checks ${esc(row.checksVersion)} → ${CURRENT_CHECKS}). This result may miss newer detections and is queued for re-scan.</div>`
		: "";
...
otter v${esc(row.scannerVersion)} · checks ${esc(row.checksVersion)}</p>
${callout}
${errors}
${findings}
${frameworksHtml(report)}
${embedHtml(row)}`;
	return layout(`${row.id} — otter`, body);
}
