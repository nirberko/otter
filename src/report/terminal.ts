...
function renderFinding(f: Finding): string {
  ...
  const loc = f.evidence.line ? `${f.evidence.source}:${f.evidence.line}` : f.evidence.source
...
export function renderTerminal(report: ScanReport): string {
  ...
  const version = report.target.resolvedVersion ? `@${report.target.resolvedVersion}` : ''
  lines.push(pc.gray(`  scanned ${layers || 'nothing'} ${version} in ${report.durationMs}ms`))
  lines.push('')
  lines.push(`  Score  ${c(pc.bold(`${report.score}/100`))}  ${c(scoreBand(report.score))}`)
  lines.push('')
  ...
  if (report.errors.length > 0) {
    ...
    for (const e of report.errors) lines.push(pc.gray(`    - ${e.scanner}: ${e.message}`))
  }
