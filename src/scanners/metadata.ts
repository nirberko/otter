...
import { toxicFlowFindings } from "../rules/toxicflow.js";
...
export interface Inventory {
	...
	// Set when a second tools/list poll disagreed with the first.
	repollDiff?: ToolListDiff;
}
...
export function analyzeInventory(inv: Inventory): Finding[] {
	...
	for (const tool of inv.tools) {
		findings.push(
			...analyzeDescription(
				{ toolName: tool.name, description: tool.description ?? "" },
				medianLen,
			),
		);

		const params = schemaParamNames(tool.inputSchema);
		const dangerous = params.filter((p) =>
			DANGEROUS_PARAM_NAMES.includes(p.toLowerCase()),
		);
	...
	findings.push(...toxicFlowFindings(inv.tools, inv.serverName));
	findings.push(...typosquatFindings(inv.serverName));

	if (inv.repollDiff) {
		const { added, removed, changed } = inv.repollDiff;
		const parts = [
		...
		findings.push({
			id: "MCP-NONDETERM-001",
			severity: "high",
			category: "tool-poisoning",
			title: "Tool list changes between consecutive polls",
			detail: `Two back-to-back tools/list calls returned different results (${parts.join("; ")}). A server that mutates its own tool set mid-session can swap in poisoned instructions after review.`,
			evidence: {
				source: `server:${inv.serverName}`,
				snippet: parts.join("; "),
			},
			confidence: 0.9,
			remediation:
				"Treat as untrusted; the tools you audit may not be the tools the model sees.",
		});
	}
			...
			return [
				{
					...
					evidence: {
						source: `package:${serverName}`,
						snippet: `${serverName} ≈ ${popular}`,
					},
					confidence: 0.7,
				},
			];
